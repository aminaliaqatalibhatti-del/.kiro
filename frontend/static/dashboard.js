  ...
  if (!trips.length) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <a href="/planner" class="btn-primary" style="display:inline-flex">Plan a Trip →</a>
      </div>`;
    return;
  }
...
function renderTripModal(trip) {
  ...
  content.innerHTML = `
    <div class="trip-modal-header">
      ...
      <a href="/planner" class="btn-primary">Edit in Planner</a>
    </div>
  ...
  if (!saved.length) {
    container.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1">
        ...
        <a href="/explore" class="btn-primary" style="display:inline-flex">Explore →</a>
      </div>`;
    return;
  }
  container.innerHTML = saved.map(d => `
    <div class="dest-card" onclick="window.location.href='/planner?dest=${encodeURIComponent(d.name)}'">
