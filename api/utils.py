...
Flask werkzeug ki jagah passlib use karta hai (Vercel compatible).
"""
import hashlib
import hmac
import os
...
def hash_password(password: str) -> str:
    """PBKDF2-SHA256 hash with random salt."""
...
def verify_password(password: str, pw_hash: str) -> bool:
    ...
    try:
        method, salt, stored = pw_hash.split('$')
        dk = hashlib.pbkdf2_hmac('sha256', password.encode(), salt.encode(), 260000)
...
def budget_level(budget: float, days: int, travelers: int) -> str:
    """Classify budget as low / medium / high."""
